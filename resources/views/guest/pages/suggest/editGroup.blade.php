...
<div class="col-10 offset-1 col-md-8 offset-md-2 my-2 my-md-1 text-center">
  <h3 class="c1">
    <strong>Predlog za promenu podataka grupe proizvoda {{$productGroup->name}} </strong>
  </h3>
...
<form action="{{route('storeEditGroupSuggestion', $productGroup->id)}}" method='post' class='w-100'>
   @csrf
    <div slot="commentEditor" class="my-4 w-">
      ...
      <textarea name="suggestion" class="form-control productDescription w-100">
        ...
        {{old('description')}}
        @else
        @edit {{$productGroup->description}} @endedit
        @enderrors
      </textarea>
