<div class="product col-10   col-sm-8  col-lg-5 offset-lg-0 pt-2 pb-4 mr-lg-4 @if($key>3 && isset($product->manufacturer_id)) d-none d-md-inline-block @endif">
  <div class="row">

    {{-- SLIKA PROIZVODA --}}
   <div class="productImage col-4 col-lg-5">
     <img class='circle' src="/images/{{$product->image}}" alt="">
   </div>
   {{-- //SLIKA PROIZVODA --}}
   ...
   <div class="col-8 col-lg-7 productQuickInfo ">
     ...
     @if(isset($product->manufacturer))
        <a class=" fo1 mb-1 text-center  w-100 c2Link  mb-2" href="{{route('manufacturerProductsGuest', $product->manufacturer->id)}}">
          <span>proizvodi </span>  <br class='d-md-none'><span class="underlined c2 c1-md">{{$product->manufacturer->name}}</span>
        </a>
     @else
       <a class=" fo1 mb-1 text-center  w-100 c2Link  mb-2" href="{{route('checkProductGroup', $product->id)}}">
